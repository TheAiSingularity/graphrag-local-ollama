...
def run_drift_search(
    ...
    """Run a DRIFT (iterative graph reasoning) search with the given query."""
    data_dir, root_dir, config = _configure_paths_and_settings(data_dir, root_dir)
    data_path = Path(data_dir)
    ...
    community_reports_path = data_path / "create_final_community_reports.parquet"
    final_community_reports = (
        pd.read_parquet(community_reports_path)
    ...
    reports = (
        read_indexer_reports(final_community_reports, final_nodes, community_level)
        if not final_community_reports.empty
        else []
    )

    covariates_path = data_path / "create_final_covariates.parquet"
    covariates = (
        {"claims": read_indexer_covariates(pd.read_parquet(covariates_path))}
    ...
    result = search_engine.search(query=query)
    reporter.success(f"DRIFT Search Response: {result.response}")
    return result.response
...
def run_basic_search(
    ...
    """Run a basic vector-similarity search with the given query."""
    data_dir, root_dir, config = _configure_paths_and_settings(data_dir, root_dir)
    data_path = Path(data_dir)
    ...
    result = search_engine.search(query=query)
    reporter.success(f"Basic Search Response: {result.response}")
    return result.response
